...
public class Entity {
	...
	public void setBoundingBoxSize(float percentageWidthReduced, float percentageHeightReduced) {
		...
		float widthReductionAmount = 1.0f - percentageWidthReduced; // .8f for
																	// 20% (1 -
																	// .20)
		float heightReductionAmount = 1.0f - percentageHeightReduced; // .8f for
																		// 20%
																		// (1 -
																		// .20)
		...
		// Need to account for the unitscale, since the map coordinates will be
		// in pixels
		float minX = _nextPlayerPosition.x;
		float minY= _nextPlayerPosition.y;
		...
		// Gdx.app.debug(TAG, "SETTING Bounding Box: (" + minX + "," + minY + ")
	...
	private void loadAllAnimations() {
		...
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 4; j++) {
				// Gdx.app.debug(TAG, "Got frame " + i + "," + j + " from " +
				// sourceImage);
	...
	public void setNextPositionToCurrent() {
		...
		// Gdx.app.debug(TAG, "Setting nextPosition as Current: (" +
		// _nextPlayerPosition.x + "," + _nextPlayerPosition.y + ")");
	}
	...
	public void calculateNextPosition(Direction currentDirection, float deltaTime) {
		...
		// Gdx.app.debug(TAG, "calculateNextPosition:: Current Position: (" +
		// _currentPlayerPosition.x + "," + _currentPlayerPosition.y + ")" );
		// Gdx.app.debug(TAG, "calculateNextPosition:: Current Direction: " +
		// _currentDirection );
